odoo.define('brain_pack_backend_ent.MenuJs', function (require) {
    'use strict';
     ...
     var qweb = core.qweb;
     var ColorPallet = require('brain_pack_backend_ent.ColorPalletJS')
     ...
     patch(NavBar.prototype, "theme_backend.MenuJs", {
         async setup(parent, menuData) {
            ...
            this.brainpack_support_url = session.session.brainpack_support_url;
         ...
         _OpenAppdrawer: function (ev) {
             ...
             if($('#oe_neutralize_banner').length > 0){
                $('#oe_neutralize_banner').parent().hide()
             }
         },
         ...
         _CloseAppdrawer: function (ev) {
             ...
             $('.appdrawer_section .apps-list .row').removeClass('d-none');
             if($('#oe_neutralize_banner').length > 0){
                $('#oe_neutralize_banner').parent().show()
             }
         },
                ...
                $.each(menu_data, function( key, value ) {
                    ...
                    if (current_record.use_icon) {
                        if (current_record.icon_class_name) {
                            ...
                            var icon_image = "<img class='img img-fluid' src='/brain_pack_backend_ent/static/description/bizople-icon.png' />"
                            $tagtarget.find('.app_icon').append($(icon_image))
                        }
                    } else {
                        if (current_record.icon_img) {
         ...
         addconfiguratorclass: function (){
             ajax.rpc('/get/model/record').then(function(rec) {
                 ...
                 if(rec.record_dict[0].font_size){
                    loadCSS(`/brain_pack_backend_ent/static/src/scss/font_sizes/${rec.record_dict[0].font_size}.css`);
                 }
        ...
        _GetLanguages: function() {
         ...
         _hideSearchbarModal: function (ev) {
            console.log('_hideSearchbarModal')
            this.$search_modal_popup.hide();
            this.$search_modal_popup.removeClass('show')
            this._searchModalReset()
         },
         ...
         _searchPages: function(){
            ...
            if ($selected_search_mainmenu_name != '0'){
             ...
             this.$search_modal_results.html(
                 core.qweb.render("brain_pack_backend_ent.MenuSearchResults", {
                     results: results,
                     widget: this,
                 })
             );

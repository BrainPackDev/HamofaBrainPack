    ...
    def extract_data(self, model, values):
        if model.sudo().model == 'helpdesk.ticket':
            ...
            for field_name, field_value in values.items():
                ...
                elif 'ticket_type_id' in field_name:
                    data['record']['ticket_type_id'] = int(field_value)

                    team_id = request.env['helpdesk.team'].search([('member_ids', 'in', request.env.uid)], limit=1).id
                    if not team_id:
                        team_id = request.env['helpdesk.team'].search([], limit=1).id
                    data['record']['team_id'] = team_id
            ...
            # data = super(WebsiteFormInherit).extract_data(model, values)
